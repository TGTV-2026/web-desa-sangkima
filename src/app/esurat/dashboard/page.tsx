  ...
  return (
    <div className="w-full text-ink select-none antialiased flex flex-col gap-4 md:gap-5">
      <PageHeader
        breadcrumb={
          isPetugas
            ? { parent: "Layanan Administrasi", current: "Beranda Staff" }
            : { parent: "Layanan Administrasi", current: "Beranda Warga" }
        }
      ...
      {session.role === "user" && !session.isProfileComplete && (
        <div className="rise-in bg-brass/5 border border-brass/30 rounded-sm px-4 py-2.5 flex items-start gap-3">
          <span className="text-brass text-[16px] md:text-lg leading-none mt-0.5 shrink-0">
            ⚠
          </span>
      ...
      <div
        className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-4 md:gap-5 items-stretch rise-in"
        ...
        <div className="card-doc bg-paper border border-line/70 rounded-sm shadow-sm overflow-hidden flex flex-col h-full">
          <div className="flex items-center justify-between px-4 py-2.5 bg-paper2/20 border-b border-line gap-4 shrink-0">
            ...
            <Link
              href={
                isPetugas
                  ? "/esurat/dashboard/permohonan"
                  : "/esurat/dashboard/surat"
              }
            ...
            <ul className="divide-y divide-line/60 flex-1">
              {recent.map((r) => (
                <LetterRequestListItem
                  ...
                  href={
                    isPetugas
                      ? `/esurat/dashboard/permohonan/${r.id}`
                      : `/esurat/dashboard/surat/${r.id}`
                  }
        ...
        <div className="card-doc bg-paper border border-line/70 rounded-sm shadow-sm p-4 relative overflow-hidden flex flex-col h-full justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 border-b border-line pb-2 mb-3">
              ...
              <h3 className="font-serif text-[16px] md:text-lg font-medium text-pine-900">
                Alur Layanan E-Surat
              </h3>
            </div>
            ...
            <ol className="relative border-l border-line/70 ml-2 space-y-3.5 text-[14px] md:text-sm text-inkmut">
              <li className="relative pl-4">
                ...
                <p className="font-semibold text-ink">
                  1. Ajukan Secara Mandiri
                </p>
                <p className="mt-0.5 leading-relaxed">
                  Pilih jenis surat yang Anda butuhkan, lalu isi formulir data
                  tambahan sesuai instruksi dokumen.
                </p>
              </li>
              <li className="relative pl-4">
                ...
                <p className="mt-0.5 leading-relaxed">
                  Petugas administrasi kantor Desa Sangkima akan memeriksa
                  kesesuaian identitas dan maksud tujuan Anda.
                </p>
              </li>
              <li className="relative pl-4">
                <span className="absolute -left-[4.5px] top-1 flex h-2 w-2 rounded-full bg-inkmut/40" />
                ...
                <p className="mt-0.5 leading-relaxed">
                  Setelah disetujui Kepala Desa, surat digital berformat PDF
                  lengkap dengan kode verifikasi resmi siap diunduh.
                </p>
              </li>
            </ol>
          </div>
          ...
          <div className="mt-4 bg-paper2/40 border border-line/50 rounded-sm p-3 text-sm leading-relaxed text-inkmut shrink-0 relative z-10">
            💡 <span className="font-medium text-ink">Butuh Bantuan?</span> Jika
            proses verifikasi memakan waktu lebih dari 1x24 jam, Anda dapat
            mendatangi Kantor Desa Sangkima dengan membawa KTP asli.
          </div>
        </div>
      </div>
    </div>
